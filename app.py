...
def main():
    with st.form("road_traffic_severity_form"):
       st.subheader("Please enter the following inputs:")
        ...
        #the inputs
       No_vehicles = st.slider("Number of vehicles involved:",1,7, value=0, format="%d")#for numerical
       No_casualties = st.slider("Number of casualties:",1,8, value=0, format="%d")
       Hour = st.slider("Hour of the day:", 0, 23, value=0, format="%d")
       collision = st.selectbox("Type of collision:",options=options_types_collision)#for categorical
...
# encode using ordinal encoder and predict
    if submit:
       ...
       encoded_arr = list(encoder.transform(input_array).ravel())#encoding the cateegorical features
